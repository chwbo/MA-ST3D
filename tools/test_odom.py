...
def eval_single_ckpt_saved(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=False, state_name='model_state'):
    # load checkpoint
    model.load_params_from_file(filename=args.ckpt, logger=logger, to_cpu=dist_test, state_name=state_name)
    model.cuda()

    # start evaluation
    eval_utils.eval_one_epoch_saved(
        cfg, model, test_loader, epoch_id, logger, dist_test=dist_test,
        result_dir=eval_output_dir, save_to_file=args.save_to_file, args=args
    )
    ...
    while True:
        ...
        if cfg.LOCAL_RANK == 0:
            for key, val in tb_dict.items():
                if key.find('diff') > -1:
                    tb_log.add_scalars(key+'_pos', {"x": val[0], "y": val[1], "z": val[2]}, cur_epoch_id)
                    tb_log.add_scalars(key+'_dim', {"w": val[3], "l": val[4], "h": val[5]}, cur_epoch_id)
                    tb_log.add_scalars(key+'_ori', {"a": val[6]}, cur_epoch_id)
                else:
...
def repeat_eval_ckpt_saved(model, test_loader, args, eval_output_dir, logger, ckpt_dir, dist_test=False, state_name='model_state'):
    # evaluated ckpt record
    ckpt_record_file = eval_output_dir / ('eval_list_%s.txt' % cfg.DATA_CONFIG.DATA_SPLIT['test'])
    with open(ckpt_record_file, 'a'):
        pass
    ...
    while True:
        ...
        # start evaluation
        cur_result_dir = eval_output_dir / ('epoch_%s' % cur_epoch_id) / cfg.DATA_CONFIG.DATA_SPLIT['test']
        tb_dict = eval_utils.eval_one_epoch_saved(
            cfg, model, test_loader, cur_epoch_id, logger, dist_test=dist_test,
            result_dir=cur_result_dir, save_to_file=args.save_to_file, args=args
        )

...
from pcdet.utils import self_training_odom_utils
...
def train_one_epoch_st(model, optimizer, source_reader, target_loader, model_func, lr_scheduler,
    ...
    verify_ps_bbox_meter = common_utils.AverageMeter()
    st_loss_meter = common_utils.AverageMeter()
    det_ver_err_meter = common_utils.AverageMeter()
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        if tb_log is not None and rank == 0:
        ...
        ver_pseudo_bbox = target_batch['ver_ps_bbox'].mean().item()
        ps_bbox_meter.update(pos_pseudo_bbox)
        ignore_ps_bbox_meter.update(ign_pseudo_bbox)
        verify_ps_bbox_meter.update(ver_pseudo_bbox)

        det_ver_err = target_batch['det_ver_err'].max(axis=0)
        det_ver_err_meter.update(det_ver_err)
        ...
        disp_dict.update({'st_loss': "{:.3f}({:.3f})".format(st_loss_meter.val, st_loss_meter.avg),
                          ...
                          'ver_ps_box': verify_ps_bbox_meter.avg,
                          'det_ver_err': det_ver_err_meter.avg})
        # merge verify errors
        ...
        if rank == 0:
            ...
            if tb_log is not None:
                ...
                tb_log.add_scalar('train/ver_ps_bbox', verify_ps_bbox_meter.val, accumulated_iter)
                for key, val in st_tb_dict.items():
                    tb_log.add_scalar('train/' + key, val, accumulated_iter)
                for i_err in range(det_ver_err.shape[0]):
                    tb_log.add_scalar('train/boxes_ver_err_{}'.format(i_err), det_ver_err[i_err], accumulated_iter)
    if rank == 0:
        pbar.close()
        ...
        tb_log.add_scalar('train/epoch_ver_ps_box', verify_ps_bbox_meter.avg, accumulated_iter)
...
def train_model_st(model, optimizer, source_loader, target_loader, model_func, lr_scheduler, optim_cfg,
    ...
    # zc revised
    ps_pkl = self_training_odom_utils.check_already_exsit_pseudo_label(ps_label_dir, start_epoch)
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True,
        ...
        for cur_epoch in tbar:
            ...
            if (cur_epoch in cfg.SELF_TRAIN.UPDATE_PSEUDO_LABEL) or \
                ...
                ret_ps_meters = self_training_odom_utils.save_pseudo_label_epoch(
                    model, target_loader, rank,
                    leave_pbar=True, ps_label_dir=ps_label_dir, cur_epoch=cur_epoch
                )
                if rank == 0:
                    tb_log.add_scalar('train/epoch_glb_ps_pos_boxes_avg', ret_ps_meters['avg_glb_ps_pos'], accumulated_iter)
                    tb_log.add_scalar('train/epoch_glb_ps_ign_boxes_avg', ret_ps_meters['avg_glb_ps_ign'], accumulated_iter)
            ...
            print('Notice: for compute time of training one epoch')
            T_0 = time.time()
            ...
            T_1 = time.time()
            print('time of train one epoch: %f ms ' % ((T_1 - T_0) * 1000))

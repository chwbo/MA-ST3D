...
class AxisAlignedOdomTargetAssigner(object):
    ...
    def assign_one_class(self, cur_gt, cur_gt_classes, all_anchors, use_multihead=False):
        target_list = []
    ...
    def assign_targets(self, all_anchors, gt_boxes_with_classes, use_multihead=False):
        ...
        for k in range(batch_size):
            ...
            ## assign the divide loss
            if self.anchor_target_cfg.NAME == 'AxisAlignedOdomTargetAssigner':
                odom_cur_gt_idx = cur_gt_classes == 2
                ## ignore odom label
                cur_gt_classes[odom_cur_gt_idx] = -1
                ## ignore static label
                odom_cur_gt_classes = -1 * torch.from_numpy(np.ones(cur_gt_classes.shape, np.int32)).cuda()
                odom_cur_gt_classes[odom_cur_gt_idx] = 1
                odom_target_dict = self.assign_one_class(cur_gt, odom_cur_gt_classes, all_anchors)

                odom_bbox_targets.append(odom_target_dict['box_reg_targets'])
                odom_cls_labels.append(odom_target_dict['box_cls_labels'])
                odom_reg_weights.append(odom_target_dict['reg_weights'])

            target_dict = self.assign_one_class(cur_gt, cur_gt_classes, all_anchors)
        ...
        if self.anchor_target_cfg.NAME == 'AxisAlignedOdomTargetAssigner':
            odom_bbox_targets = torch.stack(odom_bbox_targets, dim=0)
            odom_cls_labels = torch.stack(odom_cls_labels, dim=0)
            odom_reg_weights = torch.stack(odom_reg_weights, dim=0)

            all_targets_dict.update({
                'odom_box_cls_labels': odom_cls_labels,
                'odom_box_reg_targets': odom_bbox_targets,
                'odom_reg_weights': odom_reg_weights
            })

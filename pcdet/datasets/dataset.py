...
from ..utils import common_utils, box_utils, self_training_utils, self_training_odom_utils
...
class DatasetTemplate(torch_data.Dataset):
    ...
    def fill_pseudo_labels(self, input_dict):
        gt_boxes = None
        if self.dataset_cfg.DATASET == 'KittiOdomDataset':
            gt_boxes, det_ver_err = self_training_odom_utils.load_ps_label(input_dict['frame_id'])
            # for odom verify error
            input_dict['det_ver_err'] = det_ver_err
        elif self.dataset_cfg.DATASET == 'NuScenesOdomDataset':
            gt_boxes, det_ver_err = self_training_odom_utils.load_ps_label(input_dict['frame_id'])
            # for odom verify error
            input_dict['det_ver_err'] = det_ver_err
        else:
        ...
        idx = gt_classes == 2
        num_ver_ps_boxes = idx.sum()
        #if num_ver_ps_boxes > 0:
        #    gt_classes[idx] = 1
        gt_boxes = gt_boxes[:, :7]
        ...
        input_dict['ver_ps_bbox'] = num_ver_ps_boxes

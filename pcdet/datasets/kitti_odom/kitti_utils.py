...
def read_poses(sequ, root_dir):

    poses_file = root_dir / 'sequences' / sequ[:2] / 'poses.txt'
    poses = []
    with open(poses_file) as f:
        lines = f.readlines()
        for line in lines:
            obj = line.strip().split(' ')[:]
            pose = np.array(obj, dtype=np.float32).reshape(3, 4)
            poses.append(pose)
    return poses

def read_calib(sequ, root_dir):

    calib_file = root_dir / 'sequences' / sequ[:2] / 'calib.txt'
    with open(calib_file) as f:
        ...
        Tr = np.array(obj, dtype=np.float32).reshape(3, 4)
    calib ={
        "P0": P0,
        "P1": P1,
        "P2": P2,

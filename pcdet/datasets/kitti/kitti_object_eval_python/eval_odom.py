    ...
    if compute_fp:
        ...
        if compute_aos:
            ...
            if tp > 0 or fp > 0:
                ...
                similarity = -1
    return tp, fp, fn, similarity, thresholds[:thresh_idx], gt_dt_pair
...
def compute_localize_diff(gt_anno, dt_anno, gt_dt_pairs):
    if len(gt_dt_pairs) > 0:
        gt_boxes = np.concatenate((gt_anno['location'], gt_anno['dimensions'], gt_anno['rotation_y'][..., np.newaxis]), axis=-1)
        dt_boxes = np.concatenate((dt_anno['location'], dt_anno['dimensions'], dt_anno['rotation_y'][..., np.newaxis]), axis=-1)
        gt_idx = [gt_dt_pairs[i, 0] for i in range(len(gt_dt_pairs))]
        dt_idx = [gt_dt_pairs[i, 1] for i in range(len(gt_dt_pairs))]
        diff = gt_boxes[gt_idx] - dt_boxes[dt_idx]
        diff_mean = diff.mean(axis=0)
    else:
        diff_mean = np.zeros((7,))
    return diff_mean
...
def eval_class(gt_annos,
    ...
    diff = np.zeros([num_class, num_difficulty, num_minoverlap, 7])
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    rets = compute_statistics_jit(
                        ...
                        thresh=0.0,
                        compute_fp=False)
                    tp, fp, fn, similarity, thresholds, gt_dt_pairs = rets
                    if metric > 0:
                        gt_dt_pairs = np.array(gt_dt_pairs)
                        loc_diff = compute_localize_diff(gt_annos[i], dt_annos[i], gt_dt_pairs)
                        loc_diffs.append(loc_diff)
                    thresholdss += thresholds.tolist()
                loc_diffs = np.array(loc_diffs)
                diff[m, l, k, :] = loc_diffs.mean(axis=0)
...
def do_eval_odom(gt_annos,
    ...
    return mAP_bbox, mAP_bev, mAP_3d, mAP_aos, mAP_bbox_R40, mAP_bev_R40, mAP_3d_R40, mAP_aos_R40, diff_bev, diff_3d
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    #            compute_aos = True
    #        break
    mAPbbox, mAPbev, mAP3d, mAPaos, mAPbbox_R40, mAPbev_R40, mAP3d_R40, mAPaos_R40, diff_bev, diff_3d = do_eval_odom(
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            if compute_aos:
                ...
                if i == 1:
                    #ret_dict['%s_aos/easy' % class_to_name[curcls]] = mAPaos[j, 0, 0]
                    ret_dict['%s_aos_0.5/moderate' % class_to_name[curcls]] = mAPaos[j, 1, 1]
                    #ret_dict['%s_aos/hard' % class_to_name[curcls]] = mAPaos[j, 2, 0]
            ...
            if compute_aos:
                ...
                if i == 1:
                   #ret_dict['%s_aos/easy_R40' % class_to_name[curcls]] = mAPaos_R40[j, 0, 0]
                   ret_dict['%s_aos_0.5/moderate_R40' % class_to_name[curcls]] = mAPaos_R40[j, 1, 1]
                   #ret_dict['%s_aos/hard_R40' % class_to_name[curcls]] = mAPaos_R40[j, 2, 0]
            ...
            if i == 1:
                # ret_dict['%s_3d/easy' % class_to_name[curcls]] = mAP3d[j, 0, 0]
                ret_dict['%s_3d_0.5/moderate' % class_to_name[curcls]] = mAP3d[j, 1, 1]
                # ret_dict['%s_3d/hard' % class_to_name[curcls]] = mAP3d[j, 2, 0]
                # ret_dict['%s_bev/easy' % class_to_name[curcls]] = mAPbev[j, 0, 0]
                ret_dict['%s_bev_0.5/moderate' % class_to_name[curcls]] = mAPbev[j, 1, 1]
                # ret_dict['%s_bev/hard' % class_to_name[curcls]] = mAPbev[j, 2, 0]
                ## ret_dict['%s_image/easy' % class_to_name[curcls]] = mAPbbox[j, 0, 0]
                #ret_dict['%s_image_0.5/moderate' % class_to_name[curcls]] = mAPbbox[j, 1, 1]
                ## ret_dict['%s_image/hard' % class_to_name[curcls]] = mAPbbox[j, 2, 0]

                # ret_dict['%s_3d/easy_R40' % class_to_name[curcls]] = mAP3d_R40[j, 0, 0]
                ret_dict['%s_3d_0.5/moderate_R40' % class_to_name[curcls]] = mAP3d_R40[j, 1, 1]
                # ret_dict['%s_3d/hard_R40' % class_to_name[curcls]] = mAP3d_R40[j, 2, 0]
                # ret_dict['%s_bev/easy_R40' % class_to_name[curcls]] = mAPbev_R40[j, 0, 0]
                ret_dict['%s_bev_0.5/moderate_R40' % class_to_name[curcls]] = mAPbev_R40[j, 1, 1]
                # ret_dict['%s_bev/hard_R40' % class_to_name[curcls]] = mAPbev_R40[j, 2, 0]
                ## ret_dict['%s_image/easy_R40' % class_to_name[curcls]] = mAPbbox_R40[j, 0, 0]
                #ret_dict['%s_image_0.5/moderate_R40' % class_to_name[curcls]] = mAPbbox_R40[j, 1, 1]
                ## ret_dict['%s_image/hard_R40' % class_to_name[curcls]] = mAPbbox_R40[j, 2, 0]

            if i == 0:
                ret_dict['%s_diff_3d/moderate' % class_to_name[curcls]] = diff_3d[j, 1, 0, :]
                ret_dict['%s_diff_bev/moderate' % class_to_name[curcls]] = diff_bev[j, 1, 0, :]

            if i == 1:
                ret_dict['%s_diff_3d_0.5/moderate' % class_to_name[curcls]] = diff_3d[j, 1, 1, :]
                ret_dict['%s_diff_bev_0.5/moderate' % class_to_name[curcls]] = diff_bev[j, 1, 1, :]

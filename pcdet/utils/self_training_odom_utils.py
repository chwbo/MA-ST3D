...
from pcdet.utils import common_utils, commu_utils, memory_ensemble_odom_utils
...
def merge_with_max_boxes_max_scores(boxes, scores, merge_iou='3D', merge_cnt=3, merge_iou_thr=0.5, merge_minimum_score=None):
    ...
    for sequ in sequences:
        ...
        if cur_epoch == 0:
            ...
            if glbmem_update:
                ...
                ensemble_func = getattr(memory_ensemble_odom_utils, cfg.SELF_TRAIN.GLBMEM_ENSEMBLE.NAME)
...
def local_pathway_pseudo_label_batch(input_dict,
    ...
    for b_idx in range(batch_size):
        ...
        if locmem_update:
            ensemble_func = getattr(memory_ensemble_odom_utils, cfg.SELF_TRAIN.LOCMEM_ENSEMBLE.NAME)

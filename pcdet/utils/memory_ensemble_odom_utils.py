...
def consistency_ensemble(gt_infos_a, gt_infos_b, memory_ensemble_cfg):
    ...
    if memory_ensemble_cfg.get('IOU_BOXES', None) and memory_ensemble_cfg.IOU_BOXES == 'BEV':
    ...
    iou_mask = (ious >= memory_ensemble_cfg.IOU_THRESH_MATCH)
    ...
    #######################################################
    disappear_idx = (ious < memory_ensemble_cfg.IOU_THRESH_UNMATCH).nonzero()[0]
    ...
    newboxes_idx = (ious_b2a < memory_ensemble_cfg.IOU_THRESH_UNMATCH).nonzero()[0]
...
def soft_consistency_ensemble(gt_infos_a, gt_infos_b, memory_ensemble_cfg):
    """
    Args:
        gt_infos_a:
            gt_boxes: (N, 9) [x, y, z, dx, dy, dz, heading, label, scores]  in LiDAR for previous pseudo boxes
    ...
    new_gt_box = gt_infos_a['gt_boxes']
    new_cls_scores = gt_infos_a['cls_scores'] if gt_infos_a['cls_scores'] is not None else None
    new_iou_scores = gt_infos_a['iou_scores'] if gt_infos_a['iou_scores'] is not None else None
    new_memory_counter = gt_infos_a['memory_counter']
    ...
    discount = memory_ensemble_cfg.DISCOUNT
    # assign boxes with higher confidence
    score_mask = gt_box_selected_a[:, 8] < gt_box_selected_b[:, 8]
    if memory_ensemble_cfg.get('WEIGHTED', None):
        weight_a = gt_box_selected_a[:, 8] / (gt_box_selected_a[:, 8] + gt_box_selected_b[:, 8])
        weight_b = gt_box_selected_b[:, 8] / (gt_box_selected_a[:, 8] + gt_box_selected_b[:, 8])
        # soft merge, weighted boxes, max scores
        new_gt_box[matching_selected[:, 0], :7] = weight_a.reshape(-1,1).repeat(7, axis=1) * gt_box_selected_a[:, :7] + \
                                                  weight_b.reshape(-1,1).repeat(7, axis=1) * gt_box_selected_b[:, :7]
        #new_gt_box[matching_selected[score_mask, 0], 8] = gt_box_selected_b[score_mask, 8]
        # for matched boxes
        new_gt_box[matching_selected[:, 0], 8] = (discount * gt_box_selected_a[:, 8] + gt_box_selected_b[:, 8]) / (discount + 1)
    else:
        # hard merge, max boxes, max scores
        new_gt_box[matching_selected[score_mask, 0], :] = gt_box_selected_b[score_mask, :]
    ...
    #######################################################
    # If previous bboxes disappeared: ious < 0.2
    #######################################################
    disappear_idx = (ious < memory_ensemble_cfg.IOU_THRESH_UNMATCH).nonzero()[0]
    new_memory_counter[disappear_idx] += 1
    disappear_scores = gt_box_a[disappear_idx, 8] * discount
    # revised by zc
    new_gt_box[disappear_idx, 8] = disappear_scores

    if memory_ensemble_cfg.get('MEMORY_SCORES', None) and memory_ensemble_cfg.MEMORY_SCORES.ENABLED:
        # ignore gt_boxes that scores < ignore_thr
        ignore_mask = new_gt_box[:, 8] < memory_ensemble_cfg.MEMORY_SCORES.IGNORE_THRESH
        new_gt_box[ignore_mask, 7] = -1

        # remove gt_boxes that scores < RM_THRESH
        remain_mask = new_gt_box[:, 8] > memory_ensemble_cfg.MEMORY_SCORES.RM_THRESH
        new_gt_box = new_gt_box[remain_mask]
        new_memory_counter = new_memory_counter[remain_mask]
        if gt_infos_a['cls_scores'] is not None:
            new_cls_scores = new_cls_scores[remain_mask]
    ...
    newboxes_idx = (ious_b2a < memory_ensemble_cfg.IOU_THRESH_UNMATCH).nonzero()[0]
    if newboxes_idx.shape[0] != 0:
        # hold the boxes, increase the scores
        appear_box = gt_box_b[newboxes_idx, :]
        #appear_box[:, 8] = appear_box[:, 8]
        # revised in 20240609 for better performance, if no please roll back.
        appear_box[:, 8] = appear_box[:, 8] / discount
        # add to glb memory
        new_gt_box = np.concatenate((new_gt_box, appear_box), axis=0)
        if gt_infos_a['cls_scores'] is not None:
            new_cls_scores = np.concatenate((new_cls_scores, gt_infos_b['cls_scores'][newboxes_idx]), axis=0)
        if gt_infos_a['iou_scores'] is not None:
            new_iou_scores = np.concatenate((new_iou_scores, gt_infos_b['iou_scores'][newboxes_idx]), axis=0)
...
def nms_ensemble(gt_infos_a, gt_infos_b, memory_ensemble_cfg):
    ...
    if memory_ensemble_cfg.get('MEMORY_VOTING', None) and memory_ensemble_cfg.MEMORY_VOTING.ENABLED:
        # get ious
        if memory_ensemble_cfg.get('IOU_BOXES', None) and memory_ensemble_cfg.IOU_BOXES == 'BEV':
            iou_matrix = iou3d_nms_utils.boxes_iou_bev(gt_box_a[:, :7], gt_box_b[:, :7])
        else:
